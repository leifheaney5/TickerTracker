...
export function ChartControls() {
  ...
  const tfStyle = (active: boolean): React.CSSProperties => ({
    ...
    background: active ? 'var(--accent)' : 'transparent', color: active ? 'var(--accentInk)' : 'var(--tx2)',
  })
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 14, flexWrap: 'wrap', flex: '0 0 auto' }}>
      <div style={{ display: 'flex', gap: 3, padding: 3, borderRadius: 11, background: 'var(--card)', border: '1px solid var(--line)' }}>
        {TIMEFRAMES.map((t) => (
          <button key={t} onClick={() => setTimeframe(t)} style={tfStyle(t === timeframe)}>{t}</button>
        ))}
      </div>
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
        {compareActive && <span style={{ fontSize: '11.5px', color: 'var(--tx3)', padding: '0 4px' }}>Normalized&nbsp;%</span>}
        {!compareActive && (
          <div style={{ display: 'flex', gap: 3, padding: 3, borderRadius: 10, background: 'var(--card)', border: '1px solid var(--line)' }}>
            {CHART_TYPES.map((ct) => (
        ...
        <div style={{ position: 'relative' }}>
          <button
            ...
            style={{
              ...
              border: `1px solid ${compareActive ? 'var(--accent)' : 'var(--line)'}`,
              background: compareActive ? 'rgba(61,220,132,.1)' : 'var(--card)',
              color: compareActive ? 'var(--accent)' : 'var(--tx2)',
            }}
          >
            ⊕ Compare{compareActive ? ` (${compare.length})` : ''}
          </button>
          {menuOpen && (
            <div style={{ position: 'absolute', top: 42, right: 0, width: 200, maxHeight: 280, overflowY: 'auto', background: 'var(--panel)', border: '1px solid var(--line2)', borderRadius: 12, boxShadow: '0 18px 50px rgba(0,0,0,.55)', zIndex: 25, padding: 5 }}>
              {compare.map((s) => (
                <div key={'sel' + s} onClick={() => toggleCompare(s)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 11px', borderRadius: 8, cursor: 'pointer' }}>
                  <span style={{ fontSize: '12.5px', fontWeight: 600, color: 'var(--tx)' }}>{s}</span>
                  <span style={{ fontSize: '11px', color: 'var(--accent)' }}>✓ remove</span>
                </div>
              ))}
              {candidates.map((s) => (
                <div key={s} onClick={() => { toggleCompare(s); }} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 11px', borderRadius: 8, cursor: 'pointer' }}>
                  <span style={{ fontSize: '12.5px', fontWeight: 600, color: 'var(--tx)' }}>{s}</span>
                  <span style={{ fontSize: '11px', color: 'var(--tx3)' }}>{UNIVERSE[s]?.name ? '+ add' : '+ add'}</span>
                </div>
              ))}
              {candidates.length === 0 && compare.length === 0 && (
                authed
                  ? <div style={{ padding: '14px 11px', fontSize: '12.5px', color: 'var(--tx3)', textAlign: 'center' }}>Add tickers with the ⊕ Compare button on any chart to compare them here.</div>
                  : <div style={{ padding: '12px 11px', textAlign: 'center' }}>
                      <button onClick={() => openAuth('signup')} style={{ height: 40, padding: '0 20px', borderRadius: 11, border: 'none', background: 'var(--accent)', color: 'var(--accentInk)', fontFamily: FONT_SANS, fontSize: '13.5px', fontWeight: 700, cursor: 'pointer' }}>Sign in to compare tickers</button>
                    </div>
              )}
            </div>
          )}
