...
export function MarketViews({ sub }: { sub: Sub }) {
  ...
  const header = (title: string, sub2: string) => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <span style={{ fontSize: '21px', fontWeight: 800, letterSpacing: '-.02em', color: COLORS.tx }}>{title}</span>
      <span style={{ fontSize: '13px', color: COLORS.tx2 }}>{sub2}</span>
    ...
    <div style={{ flex: 1, overflow: 'auto', padding: 'var(--mpad,22px 26px)', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', flex: '0 0 auto' }}>
            {Object.entries(IDX).map(([key, ix]) => (
              <div key={key} style={{ flex: '1 1 0', minWidth: 170, padding: '15px 17px', borderRadius: 14, background: COLORS.card, border: `1px solid ${COLORS.line}`, display: 'flex', flexDirection: 'column', gap: 6 }}>
                <span style={{ fontSize: '11px', color: COLORS.tx3, letterSpacing: '.04em' }}>{ix.name.toUpperCase()}</span>
                <span style={{ fontFamily: FONT_MONO, fontSize: '20px', fontWeight: 600, color: COLORS.tx }}>{ix.value.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                <span style={{ fontFamily: FONT_MONO, fontSize: '12px', fontWeight: 600, color: ix.chg >= 0 ? COLORS.up : COLORS.down }}>{(ix.chg >= 0 ? '+' : '') + ix.chg.toFixed(2)}%</span>
                <div style={{ height: 3, borderRadius: 2, background: IDX_COLORS[key as keyof typeof IDX_COLORS], opacity: 0.5 }} />
              </div>
            ))}
          </div>
          <div style={{ background: COLORS.card, border: `1px solid ${COLORS.line}`, borderRadius: 16, padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 14, flex: '0 0 auto' }}>
            <span style={{ fontSize: '13px', fontWeight: 700, color: COLORS.tx }}>Sector performance · today</span>
            {SECTORS.map((s) => {
              const w = Math.min(100, Math.abs(s.chg) * 40)
              return (
                <div key={s.name} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <span style={{ width: 148, flex: '0 0 auto', fontSize: '12.5px', color: COLORS.tx, textAlign: 'right', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.name}</span>
                  <div style={{ flex: 1, height: 15, borderRadius: 4, background: COLORS.line, overflow: 'hidden' }}><div style={{ height: '100%', width: `${w}%`, background: s.chg >= 0 ? COLORS.up : COLORS.down }} /></div>
                  <span style={{ width: 62, flex: '0 0 auto', fontFamily: FONT_MONO, fontSize: '12.5px', fontWeight: 600, color: s.chg >= 0 ? COLORS.up : COLORS.down }}>{(s.chg >= 0 ? '+' : '') + s.chg.toFixed(2)}%</span>
                </div>
              )
            })}
      ...
      {sub === 'map' && (
        <>
          ...
          <div style={{ background: COLORS.card, border: `1px solid ${COLORS.line}`, borderRadius: 16, padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 12, flex: '0 0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              <span style={{ fontSize: '13px', fontWeight: 700, color: COLORS.tx }}>All sectors</span>
            ...
            <div ref={mapRef} style={{ position: 'relative', width: '100%', height: 460, background: COLORS.bg, borderRadius: 6, overflow: 'hidden' }}>
      ...
      {sub === 'sectors' && (
        <>
          ...
          <div style={{ background: COLORS.card, border: `1px solid ${COLORS.line}`, borderRadius: 16, padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 12, flex: '0 0 auto' }}>
            <span style={{ fontSize: '13px', fontWeight: 700, color: COLORS.tx }}>Performance Matrix · % change</span>
            <div style={{ overflowX: 'auto' }}>
              <div style={{ minWidth: 640 }}>
                <div style={{ display: 'grid', gridTemplateColumns: `minmax(150px,1.4fr) repeat(${SEC_TFS.length}, 1fr)`, gap: 4 }}>
                  <div />
                  {SEC_TFS.map((t) => <div key={t} style={{ textAlign: 'center', fontSize: 11, fontWeight: 600, color: COLORS.tx3, padding: '4px 0' }}>{t}</div>)}
                  {SECTORS.map((s) => (
                    <div key={s.name} style={{ display: 'contents' }}>
                      <div style={{ fontSize: 12.5, color: COLORS.tx, padding: '6px 8px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.name}</div>
                      {SEC_TFS.map((t) => {
          ...
          <div style={{ background: COLORS.card, border: `1px solid ${COLORS.line}`, borderRadius: 16, padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 14, flex: '0 0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              <span style={{ fontSize: '13px', fontWeight: 700, color: COLORS.tx }}>Ranked · {secTf} performance</span>
              <div style={{ display: 'flex', gap: 3, padding: 3, borderRadius: 10, background: COLORS.bg }}>
              ...
              {SECTORS.map((s) => ({ name: s.name, v: sectorPerf(s.name, secTf) })).sort((a, b) => b.v - a.v).map((s) => {
                ...
                return (
                  <div key={s.name} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '5px 0' }}>
                    <span style={{ width: 148, flex: '0 0 auto', fontSize: '12.5px', color: COLORS.tx, textAlign: 'right', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.name}</span>
                    <div style={{ flex: 1, height: 15, borderRadius: 4, background: COLORS.line, overflow: 'hidden' }}><div style={{ height: '100%', width: `${w}%`, background: s.v >= 0 ? COLORS.up : COLORS.down }} /></div>
                    <span style={{ width: 62, flex: '0 0 auto', fontFamily: FONT_MONO, fontSize: '12.5px', fontWeight: 600, color: s.v >= 0 ? COLORS.up : COLORS.down }}>{(s.v >= 0 ? '+' : '') + s.v.toFixed(1)}%</span>
                  </div>
                )
              })}
            </div>
