...
export function Alerts() {
  ...
  const card: React.CSSProperties = { background: 'var(--card)', border: '1px solid var(--line)', borderRadius: 16, padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 6 }

  return (
    <div style={{ flex: 1, overflow: 'auto', padding: 'var(--mpad,22px 26px)' }}>
      <div style={{ maxWidth: 760, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <span style={{ fontSize: '21px', fontWeight: 800, letterSpacing: '-.02em', color: 'var(--tx)' }}>Alerts</span>
          <span style={{ fontSize: '13px', color: 'var(--tx2)' }}>Price alerts you've set, and the symbols you're watching</span>
        </div>

        <div style={card}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
            <span style={{ fontSize: '13px', fontWeight: 700, color: 'var(--tx)' }}>Active alerts</span>
            ...
            return (
              <div key={a.symbol} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderTop: '1px solid var(--line)' }}>
                ...
                <button
                  ...
                  style={{ height: 32, padding: '0 13px', borderRadius: 8, border: '1px solid var(--line2)', background: 'transparent', color: 'var(--tx2)', fontFamily: FONT_SANS, fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}
                >
                  Remove
                </button>
              </div>
            ...
            <div style={{ padding: '24px 8px', textAlign: 'center', color: 'var(--tx3)', fontSize: '13px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
              {authed ? (
                ...
                <>
                  ...
                  <button onClick={() => openAuth('login')} style={{ height: 32, padding: '0 14px', borderRadius: 8, border: 'none', background: 'var(--accent)', color: 'var(--accentInk)', fontFamily: FONT_SANS, fontSize: '12.5px', fontWeight: 700, cursor: 'pointer' }}>Sign in</button>
                </>
              )}
            </div>
          )}
