  ...
  return (
    <div style={{ flex: 1, overflow: 'auto', padding: 'var(--mpad,22px 26px)', display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', flex: '0 0 auto' }}>
        ...
        {statCard('COINS TRACKED', <span style={{ fontFamily: FONT_MONO, fontSize: '21px', fontWeight: 600, color: 'var(--tx)' }}>{coins.length}</span>)}
          ...
          <div style={{ background: 'var(--card)', border: '1px solid var(--line)', borderRadius: 16, overflow: 'hidden' }}>
            <div style={{ overflowX: 'auto' }}>
              <div style={{ minWidth: 640 }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(150px,1.4fr) 130px 92px 130px 130px', background: 'var(--panel)', borderBottom: '1px solid var(--line)' }}>
                  {['COIN', 'PRICE', '24H', 'MKT CAP', 'DOMINANCE'].map((h) => <div key={h} style={{ padding: '12px 14px', fontSize: '11px', fontWeight: 600, letterSpacing: '.04em', color: 'var(--tx3)' }}>{h}</div>)}
                </div>
                {coins.map((c) => {
                  const up = c.change_pct >= 0
                  return (
                    <div key={c.symbol} style={{ display: 'grid', gridTemplateColumns: 'minmax(150px,1.4fr) 130px 92px 130px 130px', alignItems: 'center', borderTop: '1px solid var(--line)' }}>
